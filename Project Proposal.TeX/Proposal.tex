...
%			              & Daniel Kline\\
	...
	\section{Protocols}
		...
		\subsection{AC Current Sensor}
			The current sensor uses a transformer to induce a current between 0 and 50 mA (RMS) at 60 Hz on the secondary coil. This wave is sampled on the ATmega328P's analog input channel to calculate the RMS current though the primary coil. With the 10 bit ADC on the ATmega328P, this translates to a 98 mA (RMS) resolution on the primary coil's current. %TODO: add schematic
		
		\subsection{Barometric Pressure Sensor}
			The barometric pressure sensor communicates with the ATmega328P through an I$^2$C bus. %TODO: finsih, add schematic, register map
			
		\subsection{Brightness Sensor}
			The brightness sensors connect to the ATmega328P's ADC channels where a higher analog voltage represents a brighter room. %TODO: add schematic with multiple sensors
		
		\subsection{Sound Sensor}
			The sound sensor %TODO: start, add schematic, etc
